...
USER_CONSTRAINT_FILE_SUFFIX = "_constraints.json"
...
GUIDELINE_CATALOG = [
    ...
    GuidelineMetadata(
        guideline_id="dietary_reference_intakes",
        guideline_name="营养素参考摄入量",
        file_name="营养素参考摄入量.pdf",
        file_path=str(KNOWLEDGE_DIR / "营养素参考摄入量.pdf"),
        description="所有用户都需要同时参考的营养素约束基线。",
        applies_to_all_users=True,
    ),
]


# 直接根据 knowledge/指南映射信息.txt 固化到 Python，避免运行时解析文本。
...
UNIVERSAL_GUIDELINE_IDS = [
    item.guideline_id
    for item in GUIDELINE_CATALOG
    if item.enabled and item.applies_to_all_users
]
...
def reload_guideline_mapping_rules(mapping_path: str | Path | None = None) -> None:
    """兼容旧接口；当前映射规则已内置，不再从文本加载。"""

    return None


def get_base_guideline_rules(mapping_path: str | Path | None = None) -> list[BaseGuidelineRule]:
    """返回基础映射规则。"""

    return list(BASE_GUIDELINE_RULES)


def get_disease_guideline_rules(
    mapping_path: str | Path | None = None,
) -> list[DiseaseGuidelineRule]:
    """返回疾病附加规则。"""

    return list(DISEASE_GUIDELINE_RULES)
...
def find_candidate_guidelines(
    ...
    for metadata in lookup.values():
        if metadata.applies_to_all_users:
            candidates_with_score.append((0, metadata.guideline_id, metadata))
            continue
...
def select_guidelines_for_user(
    *,
    ...
    selected_ids = _deduplicate_ids([*base_ids, *disease_ids, *UNIVERSAL_GUIDELINE_IDS])
...
def resolve_user_constraints_path(
    user_id: str,
    knowledge_dir: str | Path | None = None,
) -> Path:
    """根据 user_id 返回 knowledge 目录下的用户约束文件路径。"""
    ...
    return base_dir / f"{normalized_user_id}{USER_CONSTRAINT_FILE_SUFFIX}"
